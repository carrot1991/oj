...
$(function(){
	$("#table").dataGrid({
		...
		fields:[  {name:'编号',field:'id',width:30,fun:level},
		          {name:'品牌编号',field:'bandid',width:60},
		          {name:'名称',field:'nickname',width:60},
		          {name:'配置',field:'config',width:60}
		          ],
	    operates:{width:130, value:[
	           {name:'删除', fun:function(row){ mydelete(row.id); } },
