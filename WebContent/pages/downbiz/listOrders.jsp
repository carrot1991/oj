	...
	$("#table").dataGrid({
		...
		operates:{width:400, value:[
									{name:'查看订单详细', fun:function(row){
										update('订单详细','pages/downbiz/ordersDetail.jsp?id='+row.id);
									}},
									{name:'打印订单', fun:function(row){
										//update('订单详细','pages/buy/myOrdersDetail.jsp?id='+row.id);
										//alert('请保存下载生成的pdf文件后；使用A4纸张进行打印。');
										updateView('订单打印','pages/buy/printOrder.jsp?id='+row.id);
									}},
	                                {name:'通过', fun:function(row){
	                                	var id=row.id;
	                                	$.get("shopord/examine?status=1&id="+id,{},function(json){
	                                ...
	                                {name:'不通过', fun:function(row){
	                                	var id=row.id;
	                                	$.get("shopord/examine?status=2&id="+id,{},function(json){
	                                		  if(json.success=='success'){
  	                                  		  	  alert("审核成功！");
  	                                  		  	  refresh();
  	                                  	  	  }else if(json.success=='-1'){
	                                ...
	                                {name:'取消定单', fun:function(row){
	                                	var id=row.id;
	                                	$.get("shopord/removeOrd",{'id':id},function(json){
	                                		  if(json.success=='success'){
  	                                  		  	  alert("取消成功！");
  	                                  		  	  refresh();
  	                                  	  	  }else{
